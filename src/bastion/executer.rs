...
pub fn execute_args(b: &mut Bastion, args: Vec<&str>) -> Result<()> {
    ...
    let app = match b.state() {
        ...
        ReplState::Request(_, _) | ReplState::EnvironmentRequest(_, _, _) => App::from(request_yaml),
    };
    ...
    //     },
        ("show", Some(matches)) => match matches.subcommand() {
            ...
            ("options", Some(matches)) => show::print_table(matches, b.get_input_options()?),
            ("workspaces", Some(matches)) => show::print_table(matches, b.get_workspaces()?),
            _ => unreachable!(),
        },
